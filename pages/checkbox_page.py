...
class CheckboxPage(BasePage):
    ...
    def click_random_checkbox(self):
        item_checkbox_list = self.elements_are_visible(self.locators.ITEM_CHECKBOX_LIST)
        for _ in range(10):
    ...
    def get_chosen_checkboxes(self) -> str:
        checked_list = self.elements_are_present(self.locators.CHECKED_CHECKBOXES)
        list_checked_names = []
        for checkbox in checked_list:
            title_checkbox = checkbox.find_element(*self.locators.TITLE_ITEM)
            list_checked_names.append(self.extract_text_from_element(title_checkbox))
    ...
    def get_output_chosen_checkboxes(self) -> str:
        resul_chosen_checkboxes = self.elements_are_present(self.locators.OUTPUT_CHOSEN_CHECKBOXES)
        list_output_checkboxes = []
        for item in resul_chosen_checkboxes:
            list_output_checkboxes.append(self.extract_text_from_element(item))
